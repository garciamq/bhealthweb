...
export const transMailBookingNew = {
    ...
    template: (data) => {
        ...
        <h4>B - Health system will automatically send email notification when confirmed appointment is complete. Thank you !</h4>`;
    },
};

export const transMailBookingFailed = {
    subject: "Email notification of booking progress at B - Health",
    template: (data) => {
        return `<h3>Thank you for booking an appointment at B - Health's system  </h3>
        <h4>Information for booked appointment:</h4>
        <div>Doctor's name: ${data.doctor} </div>
        <div>Time: ${data.time}</div>
        <div>Date: ${data.date}</div>
...
export const transMailRemedy= {
    ...
    template: (data) => {
        return `<h3>Thank you for your trust in booking a medical examination in B - Health's system.</h3>
        ...
        <br>
        <div>For example: Full name: Kurt Jimenez, with the registered phone number: 0123456789 and born: 1910, the extracted password is: <b> kurtjimenez-012-10 </b> </div>
        <br>
